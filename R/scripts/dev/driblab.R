...
df_pintar <- df_pintar %>%
  filter(Temporada=='2020/2021',Partidos>15,Posicion=="DF")
...
p<-ggplot(Pintar_Jugador, aes(x=Quartile, y=variable,group=tipo, color=RangoQuartil))+
  ...
  geom_text(aes(label=paste("\n",nTotal_Jugadores, "Defensas Big 5 Europa"), y=Inf,x=114), fontface="italic", color="grey",
            vjust=1,size=3,hjust=0.8) +
  labs(title =titulo,
       subtitle = subtitulo,
       x="",
       y="",
       caption = c("Datos por 90'", "Estilo @driblab_es elaborado por @Vdot_Spain")) +
  theme_bw()+
...
g3<-gridExtra::grid.arrange(grobs = c(player_plot), ncol = 1, as.table = FALSE,
                            top = grid::textGrob("Liam Cooper \nRendimiento 2020/2021",
                                                 gp=grid::gpar(fontsize=20,fontface="bold",col="black")))


h<-cowplot::ggdraw(g3) +
  cowplot::draw_image("cooper.png",
                      x = -0.17, y = 0.45,
                      scale=0.1
                      ) +
  cowplot::draw_image("escudo.png",
                      x = -0.15, y = 0.4,
                      scale=0.05
  ) +
