...
colors = stats_match[["general"]][["teamColors"]][["lightMode"]] %>% unlist()%>% as.data.frame()

df_normalized <- dd_final %>%
  group_by(key) %>%
  mutate(normalized_stats = stats / sum(stats) * 100)

my_colors <- c("#d2a940", "#507088")
my_colors <- c("#085098", "#C8282C")
my_colors <- alpha(colors$., 0.7)

# Crear el gráfico de barras apiladas con el mismo tamaño para todas
p <- ggplot(df_normalized, aes(x = key, y = normalized_stats, fill = name)) +
  geom_bar(stat = "identity", width = 0.7) +  # Ajustar el ancho de las barras
  geom_text(aes(label = stats), position = position_stack(vjust = 0.5), color = "white", size = 3, hjust = 0.5) +  # Ajustar la posición de los números dentro de las barras
  labs(x = NULL, y = NULL, fill = NULL) +  # Ocultar etiquetas de ejes
  # theme_minimal() +
  theme(
    panel.background = element_rect(fill = "white"),  # Fondo blanco
    panel.grid.major = element_blank(),  # Eliminar líneas de grilla
    panel.grid.minor = element_blank(),  # Eliminar líneas de grilla
...
home = seq_along(stats_match[["content"]][["lineup"]][["lineup"]][["optaLineup"]][["players"]])
seq_along(stats_match$content$lineup$lineup$optaLineup$players[[1]][[1]])

tt = stats_match$content$lineup$lineup$optaLineup$players[[1]]

dd = stats_match[["content"]][["lineup"]][["lineup"]][["optaLineup"]][["players"]][[1]][[1]][["stats"]][[1]][["stats"]] %>%
  unlist() %>%
  as.data.frame() %>%
...
tt = list(script_content$props$pageProps$content$lineup$lineup) %>% unlist()
tt[1]
tt = list(script_content$props$pageProps$content$lineup$lineup$players)
tt
...
table <- html_nodes(webpage, ".ep8dh390") %>% html_text()
table
