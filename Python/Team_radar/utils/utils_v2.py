...
def get_df_metrics(df_team, team1, team2, metric_dict, metric_to_plot):
    """
    Genera un DataFrame con métricas comparativas entre dos equipos en la misma temporada.

    Parameters:
    - df_team (pd.DataFrame): Datos de equipos en la temporada actual.
    - team1, team2 (str): Nombres de los equipos.
    - metric_dict (dict): Diccionario con nombres de métricas, columnas y tipo (1: más es mejor, 0: menos es mejor).
    - metric_to_plot (list): Lista de métricas a incluir en la comparación.

    Returns:
    - pd.DataFrame: Contiene métricas, valores por equipo y rankings percentiles relativos a la temporada.
    """

    # Crear DataFrame base
    df_metrics = pd.DataFrame(index=['games'] + metric_to_plot)

    # Agregar valores por equipo
    for team in [team1, team2]:
        df_filtered = df_team[df_team['squad'].str.replace('vs ', '') == team]
        games_played = df_filtered['games'].values[0]
        df_metrics[f'Statistic_{team}'] = [games_played] + [
            df_filtered[metric_dict[m]['m']].values[0] for m in metric_to_plot
        ]

    # Calcular percentiles sobre temporada actual
    for m_name in metric_to_plot:
        ...
        if not values.empty:
            percentiles = np.percentile(values, [5, 95]) if metric_info['type'] == 1 else np.percentile(values, [95, 5])
            df_metrics.loc[m_name, 'p5'] = percentiles[0]
            df_metrics.loc[m_name, 'p95'] = percentiles[1]
        else:
            df_metrics.loc[m_name, 'p5'] = np.nan
            df_metrics.loc[m_name, 'p95'] = np.nan

    # Ranking comparativo (entre equipos de esta temporada)
    for team in [team1, team2]:
        df_metrics[f'{team}_p'] = df_metrics.apply(
            lambda row: get_perc_by_metric(
                row[f'Statistic_{team}'],
                df_team[metric_dict[row.name]['m']] if row.name != 'games' and metric_dict[row.name]['m'] in df_team.columns else pd.Series(),
                metric_dict[row.name]['type']
            ) if row.name != 'games' else None,
            axis=1
        )
...
def get_df_metrics_old(df_team, df_team_old, team1, team2, metric_dict, metric_to_plot):
    """
...
def plot_radar(df_metrics, df_metrics2, radar_type, color_dict, date, image, radar_order=None, team_name=None):
    ...
    endnote = "EMRA - Football Data Analysis\n"
